...
export function LoginScreen({ role, onBack, onRegister, onLogin }: LoginScreenProps) {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] flex">
      {/* Left panel */}
      <div className={`hidden lg:flex w-5/12 ${cfg.bgClass} flex-col justify-between p-12 relative overflow-hidden`}>
        <div className="absolute -top-32 -left-32 w-125 h-125 rounded-full opacity-20" style={{ background: cfg.accent }} />
        <div className="absolute bottom-0 right-0 w-64 h-64 rounded-full opacity-30" style={{ background: "rgba(0,0,0,0.3)" }} />

        {/* Logo */}
        <div className="relative z-10">
          <button onClick={onBack} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm mb-12">
            ← Volver al inicio
          </button>
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-white/15 flex items-center justify-center text-2xl">{cfg.icon}</div>
        ...
        <div className="relative z-10 space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 text-white/70 text-xs font-medium border border-white/10">
            <span>{cfg.icon}</span> Acceso {cfg.label}
          </div>
          ...
          <p className="text-white/55 text-base leading-relaxed max-w-xs">{cfg.tagline}</p>
        </div>

        {/* Stats */}
        <div className="relative z-10 grid grid-cols-3 gap-3">
          {cfg.stat.map(({ num, label }) => (
            <div key={label} className="bg-white/10 rounded-2xl p-4 border border-white/10 backdrop-blur-sm">
              <p className="text-xl font-bold text-white" style={{ fontFamily: "'Georgia', serif" }}>{num}</p>
              <p className="text-white/45 text-xs mt-1 leading-tight">{label}</p>
            </div>
          ))}
        </div>
      </div>
